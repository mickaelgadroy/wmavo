...
int wiiuse_poll(struct wiimote_t** wm, int wiimotes) {
	int evnt = 0;
        ...
        //printf( "i:%d Avant evnt:%d wm[i]->event:%d, ",i, evnt, wm[i]->event ) ;
				evnt += (wm[i]->event != WIIUSE_NONE);
        //printf( "Apres evnt:%d wm[i]->event:%d\n", evnt, wm[i]->event ) ;
			} else {
				idle_cycle(wm[i]);
			}
		}
...
static void propagate_event(struct wiimote_t* wm, byte event, byte* msg)
{
  ...
  //printf( "propagate_event, event:%X\n", event ) ;
	switch (event) {
		case WM_RPT_BTN:
		{
			/* button */
	...
	/* was there an event? */
  //printf( "Avant wm->event:%x, ", wm->event ) ;
	if (state_changed(wm))
		wm->event = WIIUSE_EVENT;
  //printf( "Apres wm->event:%x\n", wm->event ) ;
}
...
static void event_data_read(struct wiimote_t* wm, byte* msg) {
  ...
  /*
  printf("Received read packet:\n");
	printf("    Packet read offset:   %i bytes\n", offset);
	printf("    Request read offset:  %i bytes\n", req->addr);
	printf("    Read offset into buf: %i bytes\n", offset - req->addr);
	printf("    Read data size:       %i bytes\n", len);
	printf("    Still need:           %i bytes\n", req->wait);
  printf("    req->buf:           %p\n", req->buf);
  printf("    req->size:           %i\n", req->size);
  printf("    msg:           %p\n", msg);
  printf("    msg+5:           %p\n", msg+5);
  ...
  */
  ...
  if( req->buf == NULL )
    puts( "events.c->event_data_read()\n Normally initialize io_nix.c -> wiiuse_connect() -> wiiuse_connect_single() -> wiiuse.c -> wiiuse_handshake() -> wiiuse_read_data_cb()" ) ;
