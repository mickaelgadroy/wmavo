...
namespace Avogadro
{
  ...
  WrapperChemicalCmdToAvoAction::WrapperChemicalCmdToAvoAction
    ( GLWidget *widget, Tool *wmtool, WmExtension *wmex, WmAvoThread *wmthread )

    : m_widget(widget), m_moleculeManip(NULL), m_wmavoThread(wmthread), 
      m_wmex(wmex),
      ...
      m_timeFirst(0), m_timeSecond(0), m_canDrawOther(false)
  {
    ...
    m_contextMenu = new ContextMenuToAvoAction( widget, m_moleculeManip, static_cast<WmTool*>(wmtool), m_wmavoThread ) ;
    ...
    bool isConnect = connect( wmtool, SIGNAL(askDistDiedre()), this, SLOT(receiveRequestToCalculDistance()) ) ;
    if( !isConnect )
      mytoolbox::dbgMsg( "Problem connection signal : wmtool.askDistDiedre() -> WrapperChemicalCmdToAvoAction.receiveRequestToCalculDistance() !!" ) ;

    isConnect = connect( this, SIGNAL(setCalculDistDiedre(Atom*)), wmtool, SLOT(calculDistDiedre(Atom*)) ) ;
    if( !isConnect )
      mytoolbox::dbgMsg( "Problem connection signal : WrapperChemicalCmdToAvoAction.setCalculDistDiedre() -> wmtool.calculDistDiedre() !!" ) ;

    isConnect = connect( this, SIGNAL(renderedSelectRect(bool,QPoint,QPoint)), wmtool, SLOT(setActiveRect(bool,QPoint,QPoint)) ) ;
    if( !isConnect )
      mytoolbox::dbgMsg( "Problem connection signal : WrapperChemicalCmdToAvoAction.renderedSelectRect() -> wmtool.setActiveRect() !!" ) ;

    // connect() must have this for non-primitive type.
    //Q_DECLARE_METATYPE(...) ; // In .h .
    qRegisterMetaType<Eigen::Vector3d>("Eigen::Vector3d") ;
    isConnect=connect( this, SIGNAL(renderedAtomBond( const Eigen::Vector3d&, const Eigen::Vector3d&, bool, bool, bool)),
                       wmtool, SLOT(renderAtomBond( const Eigen::Vector3d&, const Eigen::Vector3d&, bool, bool, bool)) ) ;
    if( !isConnect )
      mytoolbox::dbgMsg( "Problem connection signal : WrapperChemicalCmdToAvoAction.renderedAtomBond() -> wmtool.renderAtomBond() !!" ) ;    
  }
  ...
  /**
    ...
    */
  void WrapperChemicalCmdToAvoAction::transformWrapperActionToAvoAction( WmAvoThread::wmDataTransfert *wmData )
  {
    QPoint posCursor=wmData->posCursor ;
    int wmavoAction=wmData->wmActions ;
    Eigen::Vector3d pos3dCurrent=wmData->pos3dCur ;
    Eigen::Vector3d pos3dLast=wmData->pos3dLast ;
    ...
    transformWrapperActionToMoveAtom( wmavoAction, pointRef, pos3dCurrent, pos3dLast, rotCamAxeXDeg, rotCamAxeYDeg ) ;
    transformWrapperActionToMoveMouse( wmavoAction, posCursor ) ;
    transformWrapperActionToSelectAtom( wmavoAction, posCursor ) ;
    transformWrapperActionToCreateAtomBond( wmavoAction, pointRef, posCursor ) ;
    transformWrapperActionToDeleteAllAtomBond( wmavoAction ) ;
    transformWrapperActionToRemoveAtomBond( wmavoAction, posCursor ) ;

    transformWrapperActionToRotateCamera( wmavoAction, pointRef, rotCamAxeXDeg, rotCamAxeYDeg ) ;
    transformWrapperActionToTranslateCamera( wmavoAction, pointRef, distCamXTranslate, distCamYTranslate ) ;
    transformWrapperActionToZoomCamera( wmavoAction, pointRef, distCamZoom ) ;
    transformWrapperActionToInitiateCamera( wmavoAction, pointRef ) ;

    transformWrapperActionToUseContextMenu( wmavoAction, posCursor ) ;
      ...
      double rotAtomDegY )
  {
      ...
      { // Work when an atom is moving. If no movement, do not pass here.
        ...
        if( isMoved )
        {
          ...
          if( pList.size() == 1 )
          {
            ...
            if( a != NULL )
              m_wmex->adjustRumble( true, a->pos(), a ) ;
          }
        }
      }
    }
    ...
    { // Finish the action.
      ...
      if( m_isMoveAtom ) // Caution, this attribut is initialised/used in moveAtom*() methods.
      {
        ...
        m_wmex->adjustRumble( false, NULL, NULL ) ;
      ...
      //{
        ...
        { // The 2nd action.
          ...
          if( m_canDrawOther )
          {
            ...
            m_wmex->adjustRumble( true, &m_curPosDraw, NULL ) ;
