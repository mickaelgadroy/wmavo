...
namespace Avogadro {
  ...
  {
    qDebug() << "AdjustHydrogensPreCommand:constuctor()" ;
      ...
      : d(new DeleteAtomDrawCommandPrivate)
  {
    ...
    qDebug() << "  atom id:" << index << " " << "adjustHydrogens:" << adjustHydrogens ;
    Atom *atom = molecule->atom(index);
    qDebug() << " avt atom->id, atom:" << atom  ;
    qDebug() << d->id ;
    qDebug() << atom->id() ;
    d->id = atom->id();
#ifdef DEBUG_COMMANDS
  ...
  // Constructeur surchargé pour ne pas avoir à passer par l'indice de l'atome ...
  /*
  DeleteAtomDrawCommand::DeleteAtomDrawCommand(Molecule *molecule, Atom *atom, bool adjustHydrogens)
      : d(new DeleteAtomDrawCommandPrivate)
  {
    setText(QObject::tr("Delete Atom"));
    d->molecule = molecule;

    qDebug() << "  atom id:" << atom->id << " " << "adjustHydrogens:" << adjustHydrogens ;
    Atom *atom = molecule->atom(index);
    qDebug() << " avt atom->id, atom:" << atom  ;
    qDebug() << d->id ;
    qDebug() << atom->id() ;
  ...
  */
  ...
  ClearCommand::ClearCommand(Molecule *molecule, PrimitiveList selectedList) :
       m_molecule(molecule), m_selectedList(selectedList), m_originalMolecule(*molecule)
  {
  ...
  void ClearCommand::redo()
  {
    ...
    else
    {
      ...
      foreach( unsigned long residueid, m_selectedList.subList(Primitive::ResidueType) )
      {
        ...
        if( residue )
          m_molecule->removeResidue(residue);
      }
    }
    m_molecule->update();
  }
  ...
  ///////
  // Insert Command for substitute atom by fragment
  ////

  class InsertAtomByFragmentCommandPrivate {
      public:
        InsertAtomByFragmentCommandPrivate() :
          molecule(0),
          generatedMolecule(0), widget(0),
          startAtomId(-1), endAtomId(-1) {};

      Molecule *molecule;
      Molecule moleculeCopy, generatedMolecule;
      GLWidget *widget;
      int startAtomId, endAtomId ; // if we're using OBBuilder::Connect()
    };

    InsertAtomByFragmentCommand::InsertAtomByFragmentCommand(Molecule *molecule,
                                                 const Molecule &generatedMolecule,
                                                 GLWidget *widget,
                                                 const QString commandName, int startId, int endId)
        : d(new InsertAtomByFragmentCommandPrivate)
    {
      setText(commandName);
      d->molecule = molecule;
      d->moleculeCopy = *molecule;
      d->generatedMolecule = generatedMolecule;
      d->widget = widget;
      d->startAtomId = startId ;
      d->endAtomId = endId ;
    }

    InsertAtomByFragmentCommand::~InsertAtomByFragmentCommand()
    {
      delete d;
    }

    void InsertAtomByFragmentCommand::undo()
    {
      *(d->molecule) = d->moleculeCopy;
      d->molecule->update();
    }

    void InsertAtomByFragmentCommand::redo()
    {
      ...
      qDebug() << "Before add fragment" ;
      *(d->molecule) += d->generatedMolecule ;

      qDebug() << "Before get start atom" ;
      // Do we need to connect the fragment to the original molecule?
      if( d->startAtomId!=-1 && !emptyMol )
      {
        // OK, first, we should see if this atom is a hydrogen
        startAtom = d->molecule->atomById(d->startAtomId) ;
        if( startAtom->isHydrogen() )
        {
          ...
          if( hydrogen->neighbors().size() )
          {
            ...
            startAtom = d->molecule->atomById(hydrogen->neighbors()[0]) ;
          }
        ...
        {
          // heavy atom -- remove attached hydrogens
          d->molecule->removeHydrogens(startAtom);
        }


        qDebug() << "Before get end atom" ;
        /*
        if( d->endAtomId == -1 )
        { // Connect to the first atom of the fragment

          d->endAtomId = initialAtoms + 1;
        ...
        */
        ...
        { // Connect to the first atom of the fragment.
          ...
          d->endAtomId = initialAtoms + 1 ;
          endAtom = d->molecule->atomById(d->endAtomId) ;
          ...
          if( endAtom != NULL )
          {
            if( endAtom->isHydrogen() )
            {
              ...
              // Corrigé par rapport à leur version, si endAtom isH et pas de voisin, ne pas l'effacer ...
              if( hydrogen->neighbors().size() )
              {
                // the first bonded atom to this "H"

                //QList<unsigned long>bh=hydrogen->bonds() ;
                //foreach( unsigned long b, bh )
                //{
                //  qDebug() << "begin;" << d->molecule->bondById(b)->beginAtomId() << " end:" << d->molecule->bondById(b)->endAtom()->id() ;
                //}

                endAtom = d->molecule->atomById(hydrogen->neighbors()[0]);
                d->molecule->removeAtom(hydrogen);
              }
            }
            else
          ...
          {
            qDebug() << " BUG : A pointer NULL with an atom id != 0 ." ;
            qDebug() << " Info : The problem comes because Avogadro does not works correctly or works like a feet.\n'd->molecule->atomById( hydrogen->neighbors()[0] )'\nIn void InsertAtomByFragmentCommand::redo()" ;
            qDebug() << " Result : No connection realized between the molecule and the fragment." ;
          }
        }


        qDebug() << "Before add connection fragment to molecule" ;
        if( (startAtom==NULL || endAtom==NULL)
            || (startAtom->index() == endAtom->index()) )
        {
          qDebug() << " BUG : The connection between the fragment and the molecule are eguals !!" ;
          qDebug() << " Info : The problem comes because Avogadro returns wrong id values in :\n    'd->molecule->atomById(d->endAtomId) ;'\n    In void InsertAtomByFragmentCommand::redo()\n    Identical problem with the mouse." ;
          qDebug() << " Result : No connection realized between the molecule and the fragment." ;
        }
        else
        {
          qDebug() << "startAtom & endAtom : OK ..." << endl ;
          qDebug() << startAtom->index() << " " << endAtom->index() ;
          qDebug() << startAtom->id() << " " << endAtom->id() ;
      ...
      */
      ...
      if( d->widget!=NULL && endAtom!=NULL )
      {
        qDebug() << "Try selected atom" ;
        QList<Primitive *> matchedAtoms;
