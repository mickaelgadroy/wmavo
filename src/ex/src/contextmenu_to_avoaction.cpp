...
namespace Avogadro
{
  ContextMenuToAvoAction::ContextMenuToAvoAction
    ( GLWidget *widget, MoleculeManipulation *mm, WmTool *wmtool, WmAvoThread *wmavoThread )
    
    : m_widget(widget), m_wmavoThread(wmavoThread), m_moleculeManip(mm),
      ...
      m_contextMenuHydrogen(NULL), m_addAllHAct(NULL), m_removeAllHAct(NULL)
  {
    ...
    bool isConnect = connect( m_periodicTable, SIGNAL(elementChanged(int)), mm, SLOT(setAtomicNumberCurrent(int)) ) ;
    if( !isConnect )
      mytoolbox::dbgMsg( "Problem connection signal : m_periodicTable.elementChanged() -> MoleculeManipulation.setAtomicNumberCurrent() !!" ) ;

    isConnect = connect( m_periodicTable, SIGNAL(elementChanged(int)), wmtool, SLOT(setAtomicNumberCurent(int)) ) ;
    if( !isConnect )
      mytoolbox::dbgMsg( "Problem connection signal : m_periodicTable.elementChanged() -> wmtool.setAtomicNumberCurent() !!" ) ;
    ...
    if( !connectSignalToWmTool( wmtool ) )
    {
      QString msg="Bug: connectSignalToWmTool() method. At least signal is not connected." ;
      mytoolbox::dbgMsg( msg ) ;
    }
  ...
  /**
    ...
    * @return TRUE if all signals are connected ; FALSE else.
    */
  bool ContextMenuToAvoAction::connectSignalToWmTool( WmTool *tool )
  {
    bool isConnect=false, ok=true ;

    isConnect = connect( this, SIGNAL(initiatedCalculDistDiedre(int)),
                         tool, SLOT(setCalculDistDiedre(int)) ) ;
    if( !isConnect )
    {
      mytoolbox::dbgMsg( "Problem connection signal : ContextMenuToAvoAction.initiatedCalculDistDiedre() -> tool.setCalculDistDiedre() !!" ) ;
      ok = false ;
    }

    isConnect = connect( this, SIGNAL(displayedMsg(QList<QString>,QPoint)),
                         tool, SLOT(setDisplayMsg(QList<QString>,QPoint)) ) ;
    if( !isConnect )
    {
      mytoolbox::dbgMsg( "Problem connection signal : ContextMenuToAvoAction.displayedMsg() -> tool.setDisplay() !!" ) ;
      ok = false ;
    }
  ...
  void ContextMenuToAvoAction::setFontSizeContextMenu(int ratio)
  {
    ...
    */
  void ContextMenuToAvoAction::closePeriodicTable( int &wmavoAction, QPoint posCursor )
  {
  ...
  /**
    ...
    * @param posCursor The new position of the mouse cursor
    */
  void ContextMenuToAvoAction::manageAction( int &wmavoAction, QPoint posCursor )
  {
    ...
    else
    {
      ...
      closePeriodicTable( wmavoAction, posCursor ) ;
